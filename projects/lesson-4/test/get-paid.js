...
contract('Payroll', function (accounts) {
  ...
  it("Test getPaid()", function () {
    var payroll;
    return Payroll.new.call(owner).then(instance => {
      payroll = instance;
  ...
  it("Test getPaid() with insufficient fund", function () {
    var payroll;
    return Payroll.new.call(owner).then(instance => {
      payroll = instance;
      return payroll.addFund({from: owner, value: web3.toWei(fund, 'finney')});
    }).then(() => {
      return payroll.addEmployee(employee, salary, {from: owner});
    }).then(() => {
      return payroll.calculateRunway();
    }).then(runwayRet => {
      console.log('calculateRunway:'+ runwayRet);
      return web3.currentProvider.send({jsonrpc: "2.0", method: "evm_increaseTime", params: [payDuration], id: 0});
    }).then(() => {
      return payroll.getPaid({from: employee})
    }).then((getPaidRet) => {
  ...
  it("Test getPaid() by owner", function () {
    var payroll;
    return Payroll.new.call(owner).then(instance => {
      payroll = instance;
      return payroll.addFund({from: owner, value: web3.toWei(fund, 'ether')});
